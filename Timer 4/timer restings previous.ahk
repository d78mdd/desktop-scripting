...
;#notrayicon
    ...
    ;SplashTextOn,,, entering %a_scriptname%
    ;sleep, 1500
    ;SplashTextOff
...
hotkey, F6, , off
if (HOMEDRIVE="E:" or not WinExist ("D:\1 Soft' & doc's\AutoHotKey\my scripts\8in1[includes].ahk") )   ; ako e XP
  hotkey, F6, , on
*/
...
FormatTime, Y_Date, %Y_Date%, dd/MMM/yyyy

StringLower, Y_Date, Y_Date

IniRead, R, % path, % Y_Date, брой престои
IniRead, yesterday_last_hour, % path, % Y_Date, престой%R%


  StringTrimRight, yesterday_last_hour, yesterday_last_hour, 20 ; instr(chas, ":" ) - 3
  StringMid, yesterday_last_hour, yesterday_last_hour, StrLen(yesterday_last_hour) , 5, L
  StringTrimRight, yesterday_last_hour, yesterday_last_hour, 3


	;msgbox after trim`nyesterday_last_hour=%yesterday_last_hour% a_hour=%a_hour% 

difference := a_hour - yesterday_last_hour
	;msgbox after minus`na_hour - yesterday_last_hour=%difference% a_hour=%a_hour% 
MsgBox %a_hour% %yesterday_last_hour%
if (difference <= 6 && difference >= 0)	; deistviq ako razlikata mejdu posledniq v4era6en i pyrviq dne6en prestoi e po-malka ot 6 4asa
{
	;  msgbox % yesterday_last_hour
	;msgbox after check`nyesterday_last_hour=%yesterday_last_hour% a_hour=%a_hour% 
MsgBox %difference%
...
;#include %a_scriptdir%\timer - auto adjust 0.75 working.ahk
read_intervals()
after()
